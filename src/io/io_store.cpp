...
bool IOStore::loadOfferFromXml(Category* category, pugi::xml_node offer) {
   ...
   if (offer.attribute("coinType")) {
	   std::string coinTypeStr = std::string(offer.attribute("coinType").as_string());
	   if (coinTypeStr == "transferable") {
		   coinType = CoinType_t::TRANSFERABLE;
	   } else if (coinTypeStr == "coin") {
		   coinType = CoinType_t::COIN;
	   } else {
		   g_logger().warn("Offer {} has an unknown coinType: {}", name, coinTypeStr);
	   }
   }
...
void Category::addOffer(Offer newOffer) {
   for (const auto& offer : offers) {
	   if (newOffer.getOfferId() == offer.getOfferId() && newOffer.getOfferName() == offer.getOfferName()
		   && newOffer.getOfferCount() == offer.getOfferCount()
		   && newOffer.getCoinType() == offer.getCoinType()) {
		   return;
	   }
   }
