...
std::string TOTPAuthenticator::verifyToken(uint32_t accountId, uint32_t tokenTime) const {
	...
	if (accountId <= 0 || tokenTime <= 0) {
		g_logger().warn("verifyToken called with invalid accountId or tokenTime. accountId: {}, tokenTime: {}", accountId, tokenTime);
		return {};
	}
	...
	if (secret2FA.empty()) {
		g_logger().warn("Failed to retrieve secret key for accountId: {}", accountId);
		return {};
	}
	...
	try {
		...
		g_logger().warn("Error decoding secret key for accountId {}: {}", accountId, e.what());
		return {};
	}
